...
-- FOV Circle nativo (parche móvil del POV - SOLO modo FOV)
local _FovFrame = Instance.new("Frame")
_FovFrame.Name = "FOVCircleNative"
_FovFrame.AnchorPoint = Vector2.new(0.5, 0.5)
_FovFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
_FovFrame.BackgroundTransparency = 0.999
_FovFrame.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
...
_FovStroke.Thickness = 2
_FovStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
...
_sC(_RunS.RenderStepped, function()
    ...
    if _FovFrame then
        ...
        if isVis then
            -- FIJO 70 FOV (140px) para que siempre se vea en Delta
            _FovFrame.Size = UDim2.new(0, 140, 0, 140)
            _FovStroke.Color = _St.lockedTarget and _Cfg.Theme.FOVLocked or _Cfg.Theme.FOVNormal
            _FovStroke.Thickness = 2.5
            _Cfg.AimFOV = 70  -- forzar detección al mismo tamaño
        end
    end
    if _FOVCircle then
        pcall(function()
            _FOVCircle.Visible = false -- usamos Frame nativo del POV en modo FOV
        end)
    end
end)
...
FovFrame.Name = "FOVCircleNative"
FovFrame.AnchorPoint = Vector2.new(0.5, 0.5)
FovFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
FovFrame.BackgroundTransparency = 0.999
...
local function aFFA(alpha)
    ...
    if FOVCircle then
        pcall(function() FOVCircle.Radius = Config.AimFOV end)
    end
end
local function uFSU()
...
sC(RunService.RenderStepped, function(dt)
    ...
    if FovFrame then
        ...
        if isVis then
            -- FIJO 70 FOV (140px) para que siempre se vea en Delta
            FovFrame.Size = UDim2.new(0, 140, 0, 140)
            FovStroke.Color = State.lockedTarget and Config.Theme.FOVLocked or Config.Theme.FOVNormal
            FovStroke.Thickness = 2.5
            Config.AimFOV = 70  -- forzar detección al mismo tamaño
        end
    end
    if FOVCircle then
        pcall(function()
            FOVCircle.Visible = false -- usamos Frame nativo del POV en modo FOV
        end)
    end
end)
