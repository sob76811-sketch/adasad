-- =============================================================================
--    HERMANOS CP v5.4.2 ULTRA-LIGHT // INTEGRATED DELTA & XENO FIX
...
local _L = (_HCP_LANG == "en") and {
    title = "HERMANOS CP v5.4.2 // UNIFIED FIX",
    tabMain = "MAIN",
    tabCfg = "SETTINGS",
    aimbot = "AIMBOT",
    esp = "ESP VISUALS",
...
-- ==================== SECCIÓN FOV CORREGIDA (DELTA & XENO) ====================
...
local function initDrawingCircle()
    if HasDrawing then
        local success, circle = pcall(function()
            local c = Drawing.new("Circle")
            c.Thickness = 1.5
            c.NumSides = 40 -- Estabilizado para evitar bugs de renderizado en Delta
            c.Filled = false
            c.Transparency = 1
            c.Visible = false
            return c
...
local function applyFovFromAlpha(alpha)
    ...
    if FOVCircle then
        pcall(function() FOVCircle.Radius = math.max(Config.AimFOV, 5) end)
    end
end
...
safeConnect(RunService.RenderStepped, function(dt)
    if FOVCircle then
        pcall(function()
            local show = (Config.ShowFOV and Config.AimMode == "FOV")
            FOVCircle.Visible = show
            if show then
                FOVCircle.Radius = math.max(Config.AimFOV, 5)
...
print("[Hermanos CP] v5.4.2 Integrado correctamente para Delta y Xeno.")
